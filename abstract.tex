...
As a result, any spacecraft mission to an asteroid must include the ability to update the dynamic model given in situ measurements and robustness to unmodelled forces.
...
References~\cite{elmasri2005} and~\cite{sanyal2004a} investigated the coupling of an elastic dumbbell spacecraft in orbit about a central body, but only considered the case of a spherically symmetric central body.
...
As a result, the accuracy of the gravitational potential is primarily dependent on the accuracy with which the shape model represents the true surface topology.
...
Frequently, only a rough triaxial ellipsoid model is possible from the ground and an accurate shape must be determined only after a spacecraft has rendezvoused with the asteroid.
As a result, on arrival the gravitational environment near the asteroid is unknown as the shape of the asteroid is poorly modeled.
...
Using Hamilton's principle gives the inertial equations of motion of the dumbbell spacecraft as
\begin{align}
    \dot{\vb{x}} &= \vb{v}, \\
...
\end{align}
The posterior probability conditioned on the measurement is the weighted average of the prior knowledge and the measurement. 
...
Only vertices which satisfy \( \Delta \sigma_i \leq \Delta \sigma_{max} \) are considered in the Bayesian update.
...
It is possible to incorporate more vertices, for example by splitting faces into numerous other faces, but this does not affect the general approach.
...
\begin{figure}[h]
    \centering
    \subcaptionbox{Initial mesh estimate\label{fig:start}}{\includegraphics[width=0.5\textwidth]{figures/partial_initial.jpg}}~
    \subcaptionbox{First vertex added\label{fig:first}}{\includegraphics[width=0.5\textwidth]{figures/partial_0.jpg}}

    \subcaptionbox{\SI{25}{\percent} of vertices added\label{fig:quarter}}{\includegraphics[width=0.5\textwidth]{figures/partial_512.jpg}}~
    \subcaptionbox{\SI{50}{\percent} of vertices added\label{fig:half}}{\includegraphics[width=0.5\textwidth]{figures/partial_1024.jpg}}

\subcaptionbox{\SI{75}{\percent} of vertices added\label{fig:threequarter}}{\includegraphics[width=0.5\textwidth]{figures/partial_1536.jpg}}~
