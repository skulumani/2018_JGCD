...
This shape can be a coarse estimate computed from radar measurements or it can be a triaxial ellipsoid based on the semimajor axes of the asteroid.
This model is defined as a sequence of vertices and triangular facets defined by the vertices.
...
We assume the spacecraft contains a range sensor, such as LIDAR, that allows for the accurate measurement of the relative distance between the spacecraft and asteroid.
...
Model a lidar on spacecraft which gives vector of the surface in spacecraft fixed frame. 

This vector is transformed to the asteroid frame using \( R_A \).

Present algorithm for mesh update
